...
describe('CatalogService — directly-defined entity (issue #21081)', () => {
  describe('describe', () => {
    it('finds the Note entity', async () => {
      const { callTool } = mcpClient()
      const { content, error } = await callTool('describe', {})
      expect(error).to.be.null
      expect(content.entities).to.have.property('Note')
    })
  })
  ...
  describe('query — unqualified entity name on directly-defined entity', () => {
    ...
    it('unqualified and qualified return the same results', async () => {
      const { callTool } = mcpClient()
      const { content: unqualified } = await callTool('query', { cql: 'SELECT ID FROM Note' })
      const { content: qualified } = await callTool('query', {
        cql: 'SELECT ID FROM CatalogService.Note'
      })
