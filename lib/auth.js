...
const { getFilteredEntities, getFilteredActions } = require('./utils/tools-shared.js')
...
function getAccessibleEntities(srv, user) {
  ...
  for (let each in entities)
    !(function _check(name, is_root_entity) {
      const entity = entities[name]
      if (!entity) return
      if (is_root_entity && checkEntityReadAccess(entity, user))
        return // has access
...
function getAccessibleActions(srv, user) {
  return getFilteredActions(srv, (op) => checkActionAccess(op, user))
}
