...
const fmt = obj => inspect(obj, { depth: 4, compact: 3, breakLength: 80, colors: true })

const formatResult = cds.env.mcp?.format_json ? JSON.stringify : formatAsToon
...
function formatAsToon(data) {
  try {
    ...
    throw new Error(
      ...
      'Or enable JSON format with: cds.env.mcp.format_json = true'
    )
  }
...
function validateFields(fields, entity, definitions) {
  ...
  return fields.filter(field => {
    // Skip aggregate expressions and other non-ref entries
