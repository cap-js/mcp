...
async function executePerActionTool(srv, actionName, action, args) {
  LOG(actionName, { service: srv.name, ...buildQueryArgs(args) })
...
async function executeGenericReadTool(srv, entities, args) {
  ...
  LOG('query', { service: srv.name, ...buildQueryArgs(args) })
...
async function executeReadTool(srv, entityName, entity, args, options = {}) {
  if (!options.skipLog) {
    LOG(`query_${entityName}`, { service: srv.name, ...buildQueryArgs(args) })
  }
...
async function executeCallActionTool(srv, actions, args) {
  ...
  LOG('call_action', { service: srv.name, action: actionName, parameters: buildQueryArgs(parameters) })
