...
function getInstructions(def, locale) {
  locale = locale || cds.context?.locale || 'en'
  return resolveI18n(def['@mcp.instructions'], locale) || DEFAULT_INSTRUCTIONS
}
...
function createPerActionToolDefinition(actionName, action, serviceName, model) {
...
// Strip UI/internal annotations for cleaner CDL output
