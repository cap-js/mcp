  ...
  try {
    ...
    if (err.code === 401 || err.code === 403) {
      return errorResponse(
        `Authorization error (${err.code}): Not authorized to call ${actionName}. ${err.message}`
      )
    }
...
async function executeReadTool(srv, entityName, args, options = {}) {
  ...
  try {
    ...
    if (err.code === 401 || err.code === 403) {
      return errorResponse(
        `Authorization error (${err.code}): You are not authorized to read ${entityName}. ${err.message}`
      )
    }
