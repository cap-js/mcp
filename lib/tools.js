    ...
    return {
      ...
      inputSchema: z.object({
        sql: z
          .string()
          .describe(
            'CAP CQL (a superset of SQL) SELECT statement to execute. Only SELECT statements are allowed. LIMIT is auto-injected to the service max. Only standard CAP/OData functions and aggregates are permitted (count, sum, avg, min, max, lower, upper, substring, year, month, day, etc.). Database system functions (CURRENT_USER, SESSION_USER, SYSUUID, CURRENT_SCHEMA) are not supported. Use path expressions to follow associations instead of JOINs (e.g. author.name), and postfix expand syntax for nested results (e.g. SELECT from Books {title, author {name}}).'
          )
      }),
      annotations: {
        readOnlyHint: true,
...
async function _executeGenericReadSql(srv, entities, args, { log = LOG } = {}) {
  ...
  let cqn
  try {
    cqn = cds.parse.cql(sql)
  } catch (err) {
    log.error('SQL parse failed', { sql, error: err.message })
  ...
  if (!res.ok) {
    return _queryErrorResponse(
      res,
      { serviceName: srv.name, targetLabel: null, execPrefix: 'Error executing SQL' },
      log
    )
  }

  const structured = { sql, count: res.count, data: res.data }
  return {
    content: [{ type: 'text', text: formatResult(structured) }],
    structuredContent: structured
  }
