    ...
    return {
      ...
      inputSchema: z.object({
        sql: z
          .string()
          .describe(
            'SQL SELECT statement to execute. Only SELECT statements are allowed. You must set a LIMIT!'
          )
      }),
      annotations: {
        readOnlyHint: true,
...
async function executeGenericReadTool(srv, entities, args, { log = LOG } = {}) {
  if (cds.env.mcp?.format === 'sql') {
    return _executeGenericReadSql(srv, entities, args, { log: LOG })
  }
  return _executeGenericReadCqn(srv, entities, args, { log: LOG })
}
...
async function _executeGenericReadSql(srv, entities, args, { log = LOG } = {}) {
  // Normalize multiline SQL — CQL parser treats newlines as statement terminators
  const sql = args.sql?.replace(/\n/g, ' ')
  ...
  try {
    ...
    if (!validation.valid) {
      return errorResponse(
        `Error: Entity '${validation.entity}' cannot be resolved for service ${srv.name}. Use describe to see available entities.`
      )
    }
...
async function _executeGenericReadCqn(srv, entities, args, { log = LOG } = {}) {
  ...
  return executeReadTool(srv, entityName, args, { skipLog: true, log })
}
...
async function executeReadTool(srv, entityName, args, options = {}) {
  ...
  try {
    ...
    log.error('Tool execution failed', { tool: `query_${entityName}`, error: formatError(err) })

    // Handle authorization errors
    if (err.code === 401 || err.code === 403) {
      return errorResponse(
        `Authorization error (${err.code}): You are not authorized to read ${entityName}. ${formatError(err)}`
      )
    }

    return errorResponse(`Error reading ${entityName}: ${formatError(err)}`)
  }
}
