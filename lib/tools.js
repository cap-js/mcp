...
// Lazy-loaded toon encoder for optional TOON format support
let toonEncode = null

function formatAsToon(data) {
  if (!toonEncode) {
    try {
      toonEncode = require('@toon-format/toon').encode
    } catch (err) {
      throw new Error(
        'TOON format is enabled but @toon-format/toon is not installed. ' +
        'Please install it with: npm install @toon-format/toon'
      )
    }
  }
...
function formatResult(data) {
  if (cds.env.features?.mcp_format_toon === true) {
