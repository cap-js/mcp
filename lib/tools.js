...
const { formatResult, formatError, errorResponse } = require('./utils/format')
    ...
    return {
      ...
      inputSchema: z.object({
        sql: z
          .string()
          .describe(
            'SQL SELECT statement to execute. Only SELECT statements are allowed. LIMIT is auto-injected/clamped to the service max if omitted or exceeded.'
          )
      }),
      annotations: {
        readOnlyHint: true,
...
async function _executeGenericReadSql(srv, entities, args, { log = LOG } = {}) {
  ...
  const localName =
    targetName && srv.name && targetName.startsWith(srv.name + '.')
      ...
      : targetName
  const entityDef = localName ? entities[localName] : null
