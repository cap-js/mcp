...
function createDescribeToolDefinition(entityNames, actionNames, serviceName) {
  ...
  if (actionNames.length > 0) {
    schemaFields.action = z.enum(actionNames).optional()
      .describe('Specific action or function to describe. If omitted, describes all actions and functions.')
  }

  return {
    name: 'describe',
    description: `Describe the data model of ${serviceName} service. ` +
      `Returns all entities and unbound actions and functions by default. ` +
      `Use 'entity' to get only that entity's details, or 'action' to get only that action's/function's details. ` +
      `Specify both to get details about a specific entity and action/function.`,
...
function createCallActionToolDefinition(actionNames, serviceName) {
  return {
    name: 'call_action',
    description: `Call an unbound action or function in ${serviceName} service. Use describe_model to discover available actions and their parameters.`,
...
async function executeDescribe(srv, entities, actions, args) {
  ...
  if (includeEntities) {
    ...
    for (const entityName of entityNames) {
      ...
      // Resolve query limits for this entity
  ...
  if (includeActions) {
    ...
    for (const actionName of actionNamesToDescribe) {
      ...
      if (!action) continue
      const actionDoc = action.doc || action['@description'] || action['@title'] || `${action.kind} ${actionName}`

      // Determine return type: simple type, array (items), or structure (elements)
      let returns = null
      if (action.returns) {
        if (action.returns.type) {
          returns = action.returns.type
        } else if (action.returns.items) {
          returns = { array: action.returns.items.type || 'object' }
      ...
      for (const [paramName, param] of Object.entries(action.params || {})) {
        const paramDoc = param.doc || param['@description'] || param['@title'] || null
