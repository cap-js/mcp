...
function createPerActionInputSchema(action) {
  ...
  for (const [paramName, param] of Object.entries(action.params || {})) {
    ...
    // Add description from @description, @title, or doc (append enum/range/format info)
    const parts = [param['@description'] || param['@title'] || param.doc, enumDescSuffix, rangeDescSuffix, formatDescSuffix].filter(Boolean)
    if (parts.length > 0) {
