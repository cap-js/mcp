...
const { resolveI18n } = require('./tools-shared')
...
// CDS session variables allowed in where clauses (not entity fields)
const CDS_SESSION_VARIABLES = new Set(['$now', '$user'])
...
const { ALLOWED_FUNCTIONS } = require('./validate-targets')

// Portable function names — same allowlist used by CQL mode validator.
// Kept in sync with ALLOWED_FUNCTIONS in validate-targets.js.
const PORTABLE_FUNCTIONS = [...ALLOWED_FUNCTIONS]
...
const funcArgSchema = z.lazy(() =>
  z.union([z.literal('*'), refSchema, valSchema, funcSchema, xprSchema])
)
const funcSchema = z.object({
  func: z.enum(PORTABLE_FUNCTIONS).describe('Portable function name (case-sensitive)'),
  args: z
    .array(funcArgSchema)
    .describe('Function arguments: expressions, "*" for count(*), or field references'),
  as: z.string().optional().describe('Result column alias')
})

// Forward declaration for recursive expression token type (xo)
const xoSchema = z.lazy(() =>
  z.union([refSchema, valSchema, listSchema, xprSchema, funcSchema, operatorSchema, keywordSchema])
)

// List expression: { list: [expr, expr, ...] }
const listSchema = z.object({
  list: z.array(z.lazy(() => z.union([refSchema, valSchema, listSchema])))
})
...
- [{ "ref": ["createdAt"] }, "<=", { "ref": ["$now"] }] - compare with current timestamp
- [{ "ref": ["createdBy"] }, "=", { "ref": ["$user"] }] - filter by current user`
...
function createReadInputSchema(options = {}) {
  ...
  if (entityNames?.length > 0) {
    schema.entity = z.enum([...entityNames]).describe('The entity to query')
  }

  schema.where = whereClauseSchema
    .optional()
    .refine((val) => !val || JSON.stringify(val).length <= MAX_CLAUSE_LENGTH, {
  ...
  schema.groupBy = z
    ...
    .optional()
    .describe(
      'Field(s) to group results by for aggregation queries. Use "field" for simple fields or "assoc.field" for association paths. Use with aggregate functions in select.'
    )
    .refine((val) => !val || JSON.stringify(val).length <= MAX_CLAUSE_LENGTH, {
      message: `group by clause exceeds maximum length of ${MAX_CLAUSE_LENGTH} characters`
    })
  schema.having = whereClauseSchema
    .optional()
    .describe(HAVING_DESCRIPTION)
    .refine((val) => !val || JSON.stringify(val).length <= MAX_CLAUSE_LENGTH, {
      message: `having clause exceeds maximum length of ${MAX_CLAUSE_LENGTH} characters`
    })
  schema.limit = z.number().int().min(1).optional().describe('Maximum number of results to return')
  schema.offset = z
    .number()
    .int()
    .min(0)
    .optional()
    .describe(
      'Number of results to skip before returning rows (OFFSET / $skip). Use together with limit and orderBy for stable pagination.'
    )
  schema.orderBy = z
    .array(orderSchema)
    .optional()
  ...
  schema.search = z
    ...
    .optional()
    .describe(
      'Full-text search term. Searches across all string elements of the entity. Use this for fuzzy or keyword-based lookups instead of constructing complex where clauses.'
    )
  schema.distinct = z.boolean().optional().describe('Return only unique/distinct rows')
  schema.one = z
    .boolean()
    .optional()
  ...
  for (const token of whereArray) {
    if (token && typeof token === 'object') {
      ...
      if (token.ref && Array.isArray(token.ref) && token.ref.length > 0) {
        ...
        if (fieldName && !CDS_SESSION_VARIABLES.has(fieldName)) fields.push({ ref: [fieldName] })
...
function createActionInputSchema(actionNames) {
  ...
  return z.object({
    action: z.enum(actionNames).describe('The action or function to call'),
    parameters: parametersSchema
  })
}
...
function buildZodType(param, model, depth) {
  ...
  if (param.type) {
    ...
    const typeDef = model.definitions[param.type]
    if (typeDef) {
...
function buildZodTypeForItems(items, model, depth) {
  ...
  if (items.type) {
    ...
    const typeDef = model.definitions[items.type]
    if (typeDef) {
      if (typeDef.elements) {
...
function createPerActionInputSchema(action, model) {
  ...
  for (const [paramName, param] of Object.entries(action.params || {})) {
    ...
    const isScalar =
      ...
      !param.elements &&
      (!param.type ||
        CDS_TO_ZOD_TYPE_MAP[param.type] ||
        (() => {
          const typeDef = model.definitions[param.type]
    ...
    if (param.enum && isScalar) {
      ...
      const enumDesc = Object.entries(param.enum)
        .map(([k, { val }]) => `${k}=${val}`)
        .join(', ')
    ...
    const parts = [getDescription(param), enumDescSuffix, rangeDescSuffix, formatDescSuffix].filter(
      Boolean
    )
    if (parts.length > 0) {
