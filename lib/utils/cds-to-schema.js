...
function _cds2Zod(cdsType, element = {}) {
  ...
  const desc = element['@description'] || element['@title']
  if (desc) {
...
function cdsEntityToReadOutputSchema(entity, entityName) {
  ...
  for (const [name, elem] of Object.entries(entity.elements || {})) {
    ...
    if (elem.target) {
      elements[name] = z.any().optional().describe(`Association to ${elem.target}`)
      continue
    }

    elements[name] = _cds2Zod(elem.type, elem).optional()
  }
