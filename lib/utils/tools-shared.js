...
exports.getFilteredActions = function (srv, filter = () => true) {
  const actions = {}
  if (!srv.actions) return actions
  for (const op of srv.actions) {
    if (op['@cds.api.ignore']) continue
    if (!filter(op)) continue
    if (op.name.startsWith(srv.name)) actions[op.name.slice(srv.name.length + 1)] = op // use local name
  }
  return actions
}
...
exports.resolveI18n = function (value, locale) {
  ...
  if (value.startsWith('{i18n>')) {
    return cds.i18n.labels.texts4?.(locale)?.[value.slice(6, -1)] || value
  }
  return value
}
