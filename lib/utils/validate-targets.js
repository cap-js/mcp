...
const ALLOWED_FUNCTIONS = new Set([
  // Aggregates
  'count', 'countdistinct', 'sum', 'avg', 'average', 'min', 'max',
  // String search
  'concat', 'contains', 'endswith', 'indexof', 'length', 'startswith',
  'substring', 'matchespattern',
  // String manipulation
  'tolower', 'toupper', 'trim', 'lower', 'upper',
  // Date/Time parts
  'date', 'day', 'fractionalseconds', 'hour', 'maxdatetime', 'mindatetime',
  'minute', 'month', 'second', 'time', 'totaloffsetminutes', 'totalseconds', 'year',
  // Server date/time (safe)
  'now', 'current_date', 'current_time', 'current_timestamp',
  'current_utcdate', 'current_utctime', 'current_utctimestamp',
  // Arithmetic
  'ceiling', 'floor', 'round', 'ceil', 'abs', 'mod', 'power', 'sqrt',
  // Conditional / null handling
  'case', 'coalesce', 'nullif', 'cast',
  // HANA temporal diff (CAP-advertised)
  'nano100_between', 'seconds_between', 'days_between', 'months_between', 'years_between'
])

// DB pseudo-columns that parse as bare refs `{ref: ['NAME']}` (case-insensitive)
const DENIED_REFS = new Set([
  'current_schema', 'current_database', 'current_connection',
  'current_transaction_isolation_level', 'session_user', 'current_user'
])
...
function _validateSelect(select, allowed, serviceName, definitions, rootEntity) {
  ...
  const colResult = _validateExprList(
    select.columns, allowed, serviceName, definitions, contextEntity
  )
  if (!colResult.valid) return colResult

  const whereResult = _validateExprList(
    select.where, allowed, serviceName, definitions, contextEntity
  )
  if (!whereResult.valid) return whereResult

  const havingResult = _validateExprList(
    select.having, allowed, serviceName, definitions, contextEntity
  )
  if (!havingResult.valid) return havingResult

  const orderResult = _validateExprList(
    select.orderBy, allowed, serviceName, definitions, contextEntity
  )
...
function _resolveFromEntity(from, definitions, serviceName) {
  ...
  if (Array.isArray(from.ref) && typeof from.ref[0] === 'string') {
    ...
    return (
      definitions[name] ??
      (serviceName ? definitions[`${serviceName}.${name}`] : null)
    )
  }
  return null
}
  ...
  for (const item of list) {
    ...
    if (item.args) {
      if (Array.isArray(item.args)) {
        const result = _validateExprList(item.args, allowed, serviceName, definitions, contextEntity)
        if (!result.valid) return result
      } else if (typeof item.args === 'object') {
        const result = _validateExprList(
          Object.values(item.args), allowed, serviceName, definitions, contextEntity
        )
        if (!result.valid) return result
      }
    }
