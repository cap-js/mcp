...
function generateToolsForService(def) {
  ...
  const generateGenericReadTool = cds.env.features?.generic_read_tool === true

  if (generateGenericReadTool) {
    // Generate single generic read_query tool
    ...
    // Generate per-entity read tools
