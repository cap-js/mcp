...
const { getFilteredEntities, getInstructions } = require('./utils/tools-shared')
...
function resolveRefsToAbsolutePaths(schema, toolIndex) {
  ...
  jsonStr = jsonStr.replace(/#\/definitions\//g, `#/tools/${toolIndex}/inputSchema/definitions/`)
...
function generateToolsForService(def, model) {
  ...
  const entityNames = Object.keys (getFilteredEntities(def))
  ...
  tools.push({
    ...
    inputSchema: resolveRefsToAbsolutePaths(
      z.toJSONSchema(toolDef.inputSchema, { target: 'draft-07' }),
      tools.length
    ),
    ...
    if (usePerActionTools) {
      ...
      for (const [actionName, action] of Object.entries(actions)) {
        ...
        tools.push({
          ...
          inputSchema: resolveRefsToAbsolutePaths(
            z.toJSONSchema(actionDef.inputSchema, { target: 'draft-07' }),
            tools.length
          ),
...
function _compileService(def, model, options) {
  ...
  if (process.stdout.isTTY || options.as === 'json') return serverCard
  else return JSON.stringify(serverCard, null, 2)
}
