...
const { LOG, errorResponse, formatError } = require('../utils/tools-shared')
...
function registerGenericReadTool(server, srv, entities, prefix) {
  ...
  if (entityNames.length === 0) {
    LOG.debug(srv.name, '-', 'No entities to register tools for')
    return
  }
  ...
  LOG.debug(srv.name, '-', 'Registered generic tool', { tool: def.name })
}
...
async function executeGenericReadTool(srv, entities, args) {
  ...
  return _executeGenericReadCql(srv, entities, args)
}
...
function _queryErrorResponse(res, ctx) {
...
async function _executeGenericReadCql(srv, entities, args) {
  ...
  // REVISIT: remove when fixed in cds runtime
  // Qualify unqualified entity name so srv.run() can resolve it
  if (srv.name && localName && localName === targetName && entities[localName]) {
    const qualified = `${srv.name}.${localName}`
    if (fromRef.id) fromRef.id = qualified
    else cqn.SELECT.from.ref[0] = qualified
  }
  ...
  if (!res.ok) {
    return _queryErrorResponse(res, {
      serviceName: srv.name,
      targetLabel: null,
      execPrefix: 'Error executing CQL'
    })
  }
...
async function _executeGenericReadCqn(srv, entities, args) {
  ...
  if (!res.ok) {
    return _queryErrorResponse(res, {
      ...
      execPrefix: `Error reading ${entityName}`
    })
  }
