...
const { LOG, errorResponse, formatError, describeReturns } = require('../utils/tools-shared')
...
function registerCallActionTool(server, srv, actions, prefix) {
  ...
  LOG.debug(srv.name, '-', 'Registered tool', { tool: def.name, actions: actionNames })
}
...
function registerPerActionTools(server, srv, actions, prefix) {
  ...
  if (actionEntries.length === 0) {
    LOG.debug(srv.name, '-', 'No actions to register tools for service')
    return
  }
...
async function executeCallActionTool(srv, actions, args) {
  ...
  LOG(srv.name, '-', 'call', { action: actionName, parameters })
