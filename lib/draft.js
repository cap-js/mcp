...
function registerCombinedDraftTools(server, srv, draftEntities, prefix) {
  ...
  const combinedDefs = [
    {
      ...
      description: 'Activate a draft entity (make it the active version). Specify the entity and its keys.',
      _requiresHITL: true,
      annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false }
    },
    {
      op: 'edit',
      toolName: 'edit-draft',
      description: 'Put an active entity into edit mode (creates a draft copy). Specify the entity and its keys.',
      _requiresHITL: false,
      annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false }
    },
    {
  ...
  for (const def of combinedDefs) {
    ...
    const inputSchema = z.object({
      entity: z.enum([entityPaths[0], ...entityPaths.slice(1)]).describe('The draft entity to operate on'),
      keys: z.object({}).passthrough().optional().describe('Entity keys (e.g. {ID: 1}). Use describe to see key fields per entity.'),
      data: z.object({}).passthrough().optional().describe('Fields to set. Use describe to see writable fields per entity.')
    })
