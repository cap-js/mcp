...
const { formatResult, errorResponse } = require('./utils/format')
...
// Resolve .drafts entity via model — srv.entities misses feature-toggled entities
function _getDraftsEntity(fullEntity) {
  const model = cds.context?.model ?? cds.model
  return model?.definitions?.[fullEntity + '.drafts']
}
  ...
  if (entity['@Common.DraftRoot.ActivationAction']) {
    tools.push({
      ...
      handler: async (srv, args) => {
        ...
        const draftEntity = _getDraftsEntity(fullEntity)
        const res = await srv.send('SAVE', draftEntity || fullEntity, args)
  ...
  if (!entity['@readonly']) {
    tools.push({
      ...
      handler: async (srv, args) => {
        ...
        const draftEntity = _getDraftsEntity(fullEntity)
        const target = draftEntity || fullEntity
        return srv.send('NEW', target, {
          ...args,
...
function createChildDraftTools(
  ...
  tools.push({
    ...
    handler: async (srv, args) => {
      ...
      const draftRoot = _getDraftsEntity(fullRootEntity)?.name || fullRootEntity
      ...
      // Return single created entity (unwrap array if needed)
  ...
  tools.push({
    ...
    handler: async (srv, args) => {
      ...
      const draftRoot = _getDraftsEntity(fullRootEntity)?.name || fullRootEntity
      const ref = buildChildRef(draftRoot, parentParams, compositionPath, childKeyValues)
      const query = UPDATE.entity(ref).set(data)
      return srv.send({
        event: 'UPDATE',
        entity: childDraftEntity,
        data,
...
function registerDraftTools(server, srv, entities, prefix) {
  ...
  for (const { name, def } of draftEntities) {
    const tools = createDraftTools(name, def, srv.name, cds.context?.model || srv.model) // Use request-scoped model for feature-toggled composition targets
