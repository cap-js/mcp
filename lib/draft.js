...
/**
 * Detect whether a managed assoc `assocElem` is a backlink to a composition parent.
 * True if the assoc target has a composition element pointing back to `entity`.
 */
...
function getKeyElements(entity, model) {
  ...
  for (const [name, elem] of Object.entries(elements)) {
    ...
    if (backingAssocName) {
      ...
      if (backingAssoc?.on || backingAssocName === 'up_') continue
...
function createRootDraftTools(entityName, entity, serviceName, model) {
  ...
  tools.push({
    ...
    handler: async (srv, args) => {
      ...
      const draftEntity = srv.entities[entityName]?.drafts
      if (draftEntity) {
        // Use UPDATE query with IsActiveEntity=false to route through draft dispatch
        return srv.run(
          UPDATE(draftEntity)
            .set(data)
  ...
  tools.push({
    ...
    handler: async (srv, args) => {
      const draftEntity = srv.entities[entityName]?.drafts
      if (draftEntity) {
        return srv.run(DELETE.from(draftEntity).where({ ...args, IsActiveEntity: false }))
      }
...
function createChildDraftTools(
  ...
  // Precedence: composition element @singular > target entity @singular > derived from element name
  ...
  tools.push({
    ...
    annotations: { readOnlyHint: false, destructiveHint: false, idempotentHint: false },
    handler: async (srv, args) => {
      const { rootKeyValues, childKeyValues, data } = _splitArgsWithChild(
        args,
        parentKeyParams,
        childKeyElements
      )
  ...
  tools.push({
    ...
    annotations: { readOnlyHint: false, destructiveHint: true, idempotentHint: false },
    handler: async (srv, args) => {
      const { rootKeyValues, childKeyValues } = _splitArgsWithChild(
        args,
        parentKeyParams,
        childKeyElements
      )
