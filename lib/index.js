...
async function executeReadTool(srv, entityName, entity, args) {
  ...
  try {
    ...
    // REVISIT: can we get here since we use auth middleware in CAP?
    // Handle authorization errors
    if (err.code === 403 || err.status === 403) {
      return {
        content: [{
          type: 'text',
          text: `Authorization error: You are not authorized to read ${entityName}. ${err.message}`
        }],
        isError: true
      }
    }
...
async function executeDescribeModel(srv, entities, args) {
  ...
  for (const entityName of entityNames) {
    ...
    description.entities[entityName] = {
      // REVISIT: add @Core.Description 
      description: entity['@description'] || entity['@title'] || `Entity ${entityName} from ${srv.name}`,
      elements: {}
    }
    ...
    for (const [elemName, elem] of Object.entries(entity.elements || {})) {
      description.entities[entityName].elements[elemName] = {
        type: elem.type,
        key: elem.key || false,
        description: elem['@description'] || elem['@title'] || `Element ${elemName} from ${entityName}`,
