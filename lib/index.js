  ...
  router.post('/', async (req, res) => {
    try {
      ...
      if (entityCount > 0 || actionCount > 0) {
        const registerReadTool = cds.env.mcp?.per_entity_tool ? registerPerEntityReadTools : registerGenericReadTool;
      ...
      const transport = new StreamableHTTPServerTransport({
        ...
        enableJsonResponse: cds.env.mcp?.json_response ?? false
      })
