...
function createMcpServer(srv) {
  ...
  const useGenericReadTool = cds.env.features?.generic_read_tool === true

  if (useGenericReadTool) {
    // Register single generic read_query tool
    registerGenericReadTool(server, srv, entities)
  } else {
    // Register per-entity read tools (default behavior)
